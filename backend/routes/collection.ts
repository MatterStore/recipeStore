...
import Collection, {
    ICollection, deleteById, getById, getByUser
...
function withRecord(
    ...
    return async (
        ...
        (err, collection) => {
            ...
            } else if (matchUser && req.user.id != collection.user) {
                res
                    .status(403)
                    .json({
                        success: false,
...
router.get(
    ...
    (req: AuthenticatedRequest, res) => {
        getByUser(req.user.id, (err, list) => {
            if (err) {
                res
                    .status(500)
                    .json({ success: false, msg: "Something went wrong." });
...
router.get(
    ...
    (req: RecordRequest, res) => {
        ...
        if (collection.public || (req as any).user.id == collection.user) {
            res
                .status(200)
